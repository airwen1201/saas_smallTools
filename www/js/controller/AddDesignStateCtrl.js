/**
 * Created by admin111 on 16/7/11.
 */
Tailorpus.controller('AddDesignStateCtrl', function($scope,$state,$stateParams,$rootScope,$cordovaImagePicker,$ionicActionSheet,$cordovaCamera,$ionicPlatform,$cordovaFileTransfer,$ionicLoading,$cordovaToast,$ionicHistory,$ionicPopup,DataService) {
    ...
    $scope.inputList3=[]
    $scope.numIndex = $stateParams.num
    ...
    var reloadInputList = function (){
        ...
        console.log(angular.toJson(tempList))
    ...
    if (state==2){
        $scope.images_list = angular.fromJson($rootScope.designMaterials[$scope.numIndex].img)
    }else {
        $scope.images_list = [];
    }
    console.log(angular.toJson($scope.images_list))
    if (state==1){
        console.log(angular.toJson($rootScope.designMaterials))
...
//上传图片和添加颜色的删除函数
    ...
    $scope.doSubmit = function (){
        ...
        if (state==1){
            ...
            $rootScope.designMaterials[$scope.numIndex].type=$rootScope.materialType
            $rootScope.designMaterials[$scope.numIndex].name = $scope.inputList[0].value
            $rootScope.designMaterials[$scope.numIndex].color = $scope.inputList[1].value
            $rootScope.designMaterials[$scope.numIndex].img = $scope.images_list
            $rootScope.designMaterials[$scope.numIndex].property = property
        }
